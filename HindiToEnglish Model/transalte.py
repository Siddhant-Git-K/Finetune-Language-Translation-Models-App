...
# ============= CONFIGURATION =============
BASE_DIR        = r"C:\Langauge Translator Project\Hi-En language Translator"
MODEL_PATH      = os.path.join(BASE_DIR, "vosk-model-small-hi-0.22")   # Hindi Vosk model
TRANSLATOR_PATH = os.path.join(BASE_DIR, "my_en_translator")            # Hi→En MarianMT model
SAMPLE_RATE     = 16000
CHUNK_SIZE      = 4096
MIN_CONFIDENCE  = 0.7
MIN_WORD_LENGTH = 2
...
# TTS lock prevents overlapping speech
tts_lock = threading.Lock()

def speak_text_gtts(text):
    """Speak English text using Google TTS (natural human voice)"""
...
def speak_text_espeak(text):
    """Fallback: espeak (works offline)"""
    if not text or not text.strip():
        return

    try:
        subprocess.run(
            ['espeak', '-v', 'en+f3', '-s', '140', '-p', '60', '-a', '180', '-g', '10', text],
            check=True,
...
def speak_text(text):
    """Main TTS — prevents overlapping audio with lock"""
    with tts_lock:
...
def init_audio():
    ...
    if not os.path.exists(MODEL_PATH):
        print(f"[ERROR] Vosk Hindi model not found at: {MODEL_PATH}")
        print("  Download from: https://alphacephei.com/vosk/models")
        print("  Model needed : vosk-model-small-hi-0.22")
        sys.exit(1)

    try:
        vosk_model = Model(MODEL_PATH)
        print("[Audio] Hindi speech recognition model loaded")
    except Exception as e:
    ...
    if bluetooth_device_index is not None:
        ...
        print("[Audio] ✓ Using Bluetooth microphone")
...
def main():
    ...
    print("\n" + "="*70)
    print(" "*15 + "HINDI → ENGLISH SPEECH TRANSLATOR")
    ...
    print("\n[System Status]")
    print("  ✓ Translator model loaded  (Hindi → English)")
    print("  ✓ Hindi speech recognition ready")
    print("  ✓ Natural English voice (Google TTS)")
    print("  ✓ Bluetooth audio configured")
    print("  ✓ Confidence filtering enabled")
    print("\n[Instructions]")
    print("  • Speak clearly in Hindi")
    print("  • Natural English voice will play")
    print("\n[Commands]")
    print("  • Say 'बंद करो' or type Ctrl+C to shutdown")
    print("="*70 + "\n")
                    ...
                    if 'result' in result:
                            ...
                            try:
                                ...
                                if english_text:
                                    ...
                                    tts_thread = threading.Thread(
                                        target=speak_text, args=(english_text,)
                                    )
                        ...
                        try:
                            ...
                            if english_text:
                                print(f"[ENGLISH]   {english_text}")
                                threading.Thread(
                                    target=speak_text, args=(english_text,), daemon=True
                                ).start()
                        except Exception as e:
                            print(f"[ERROR] {e}")
